...
Decimal ans;
int n, limit, p, hi[MAX_N], tot, q[MAX_N], head, tail, sum[MAX_N], trans[15][MAX_N];
double f[15][MAX_N];
...
double slope(const node &rhsA, const node &rhsB) { return double(rhsA.y - rhsB.y) / double(rhsA.x - rhsB.x); }

Decimal calc(int i, int j) { return i == 0 ? hi[1] : (calc(i - 1, trans[i][j]) + sum[j] - sum[trans[i][j]]) / (j - trans[i][j] + 1); }

int main()
{
    scanf("%d%d%d%d", &n, &limit, &p, &hi[1]), tot = 1;
    for (int i = 2; i <= n; i++)
    {
        scanf("%d", &hi[i]);
        if (hi[i] > hi[1])
    ...
    n = tot, sort(hi + 1, hi + 1 + n), limit = min(limit, n);
    for (int i = 1; i <= n; i++)
        sum[i] = sum[i - 1] + hi[i];
    for (int i = 1; i <= n; i++)
        f[0][i] = hi[1];
    int upper = min(limit, 14);
    for (int i = 1; i <= upper; i++)
    {
        q[head = tail = 1] = 1;
        for (int j = 1; j <= n; j++)
            nodes[j] = node{j - 1, sum[j] - f[i - 1][j]};
        for (int j = 2; j <= n; j++)
        {
            node u = node{j, 1.0 * sum[j]};
            while (head < tail && slope(u, nodes[q[head]]) < slope(u, nodes[q[head + 1]]))
                head++;
            trans[i][j] = q[head], f[i][j] = double(sum[j] - sum[q[head]] + f[i - 1][q[head]]) / double(j - q[head] + 1);
            while (head < tail && slope(nodes[q[tail]], nodes[q[tail - 1]]) > slope(nodes[q[tail]], nodes[j]))
                tail--;
            q[++tail] = j;
        }
    }
    int m = n - limit + upper, pos;
    double mx = 0;
