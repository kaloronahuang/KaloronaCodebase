...
bool update_min(int *D, int i, int curt, int to, queue<int> &q)
{
    if (D[i] > min(D[curt], cost[to]))
        D[i] = min(D[curt], cost[to]), q.push(to);
}
bool update_max(int *D, int i, int curt, int to, queue<int> &q)
{
    if (D[i] < max(D[curt], cost[to]))
        D[i] = max(D[curt], cost[to]), q.push(to);
}
struct graph
{
